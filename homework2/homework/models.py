...
class LinearClassifier(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        # Flatten the input and apply a single linear layer
...
class MLPClassifierDeepResidual(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers):
            self.res_blocks.append(self._make_res_block(hidden_dim))

        # Output projection
        self.output_proj = nn.Linear(hidden_dim, num_classes)

    def _make_res_block(self, dim):
        """Create a residual block with two linear layers"""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for res_block in self.res_blocks:
            identity = x
            x = res_block(x)
            x = torch.relu(x + identity)  # Residual connection with ReLU
